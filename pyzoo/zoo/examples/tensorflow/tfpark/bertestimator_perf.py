...
if __name__ == '__main__':
    ...
    parser.add_option("--iterations", dest="iterations", type=int, default=100)

    (options, args) = parser.parse_args(sys.argv)
    sc = init_nncontext("BERT Perf")
    ...
    test_input_fn = bert_input_fn(test_rdd, options.max_seq_length, 32)
    predictions = estimator.predict(test_input_fn)

    batch_examples = test_examples[:options.batch_size]
    batch_features = convert_examples_to_features(batch_examples, label_list, options.max_seq_length, tokenizer)
    batch_input_ids = np.stack([np.array(feature.input_ids) for feature in batch_features])
    batch_input_mask = np.stack([np.array(feature.input_mask) for feature in batch_features])
    batch_segment_ids = np.stack([np.array(feature.segment_ids) for feature in batch_features])
    batch_input = [batch_input_ids, batch_input_mask, batch_segment_ids]
    for i in range(options.iterations):
        estimator.forward(batch_input, options.batch_size)
    print("Finished")
