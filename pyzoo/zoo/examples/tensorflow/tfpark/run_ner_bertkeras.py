...
def feature_to_input(feature):
    ...
    res = list()
    res.append(np.array(feature.input_ids))
    res.append(np.array(feature.mask))
    res.append(np.array(feature.segment_ids))
...
if __name__ == '__main__':
    ...
    from zoo.tfpark.text.keras import BERTNER
    model = BERTNER(len(label_list),
    ...
    if options.do_train:
        ...
        train_dataset = TFDataset.from_rdd(train_rdd,
                                           features=[(tf.int32, [options.max_seq_length]),
                                                     (tf.int32, [options.max_seq_length]),
                                                     (tf.int32, [options.max_seq_length])],
